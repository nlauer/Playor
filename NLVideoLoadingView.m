...
- (id)initWithFrame:(CGRect)frame andDelegate:(id)loadingViewDelegate
{
    ...
    if (self) {
        ...
        [contentView setCenter:CGPointMake(self.frame.size.width/2, self.frame.size.height/2 - 20)];
...
- (void)showInView:(UIView *)view withVideo:(NLYoutubeVideo *)video
{
    ...
    loadingTimer_ = [[NSTimer alloc] initWithFireDate:[[NSDate date] dateByAddingTimeInterval:30] interval:0 target:self selector:@selector(videoLoadTimedOut) userInfo:nil repeats:NO];
